<div>
  <div class="flex flex-col justify-center items-center mb-4" x-data="{ open: false}">
    <div class="w-full max-w-sm" x-show="!open">
      <input type="text" @click="open=true; $nextTick(() => { $refs.text.focus(); })" placeholder="Add a new task..." class="input input-md input-bordered w-full" />
    </div>
    <div x-cloak  x-show="open" x-transition:enter.scale @mousedown.outside="open = false"  @task-created.window="open = false" class="card card-compact bg-base-100 w-full md:max-w-sm shadow-md card-bordered" >
      <div class="card-body">         
        ...
        <div class="flex gap-1">      
          ...
          <details class="dropdown"  @mousedown.outside="$el.removeAttribute('open'); showInput = false">
            ...
            <ul  tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-1 text-sm shadow"  >
              @foreach(App\Models\TaskList::orderBy('title')->get() as $list)
              <li>
